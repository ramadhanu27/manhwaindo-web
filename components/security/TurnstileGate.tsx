...
export default function TurnstileGate({ children }: TurnstileGateProps) {
  ...
  // Use opacity transitions to prevent CLS instead of conditional rendering
  return (
    <>
      {/* Verification Gate Overlay - Hidden when verified */}
      <div
        className={`fixed inset-0 z-50 bg-gradient-to-br from-slate-900 via-purple-900/20 to-slate-900 flex items-center justify-center px-4 transition-opacity duration-300 ${isVerified ? "opacity-0 pointer-events-none" : "opacity-100"}`}
        style={{
      ...
      {/* Main Content - Always rendered to prevent CLS */}
      <div className={`transition-opacity duration-300 ${isVerified ? "opacity-100" : "opacity-0"}`}>{children}</div>
    </>
  );
}
