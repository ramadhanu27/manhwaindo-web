  ...
  return (
    ...
    >
      ...
      <div className="flex gap-3 sm:gap-4">
        {/* Thumbnail */}
        <div className="relative w-16 h-24 sm:w-20 sm:h-28 rounded overflow-hidden bg-muted flex-shrink-0">
          {type && (
        ...
        <div className="space-y-1 border-t border-border pt-2">
          {chapters.slice(0, 2).map((chapter: any, index: number) => (
            <div key={index} className="flex items-center justify-between gap-2 text-xs">
              ...
              <span className="text-muted-foreground flex-shrink-0 whitespace-nowrap text-[11px]">
                {chapter.time}
              </span>
            </div>
          ))}
