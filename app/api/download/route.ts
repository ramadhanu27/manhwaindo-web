  ...
  try {
    ...
    if (!seriesSlug || !chaptersParam) {
      ...
      return NextResponse.json(
        { error: 'Missing series or chapters parameter' },
        { status: 400 }
      );
    }
    ...
    const seriesResponse = await fetch(
      `https://rdapi.vercel.app//api/series/${seriesSlug}`,
    ...
    if (!seriesResponse.ok) {
      ...
      return NextResponse.json(
        { error: `Series not found (${seriesResponse.status})` },
        { status: 404 }
      );
    }
      ...
      try {
        ...
        try {
          ...
          const chapterResponse = await fetch(
            `https://rdapi.vercel.app//api/chapter/${cleanChapterSlug}`,
            { signal: AbortSignal.timeout(10000) }
          );
